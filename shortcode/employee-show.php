...
function employee_show() {
    ...
    if ( $results ) {
        ...
        // Create an array to hold the split intervals
        $split_intervals = [];

        foreach ( $results as $row ) {
            // Extract start and end times from the row
            $startTime = strtotime( $row->startTime );
            $endTime = strtotime( $row->endTime );

            // Loop through each 30-minute interval within the start and end time
            while ( $startTime < $endTime ) {
                // Calculate the end time for the current interval ( 30 minutes later )
                $interval_end = strtotime( '+30 minutes', $startTime );

                // If the calculated end time exceeds the actual end time, set it to the end time
                if ( $interval_end > $endTime ) {
                    $interval_end = $endTime;
                }

                // Create a formatted interval key ( e.g., 'HH:MM-HH:MM' )
                $interval_key = date( 'H:i', $startTime ) . '-' . date( 'H:i', $interval_end );

                // Store data within the respective interval key
                if ( !isset( $split_intervals[ $interval_key ] ) ) {
                    $split_intervals[ $interval_key ] = [];
                }

                // Store the row data within the interval key
                $split_intervals[ $interval_key ][] = $row;

                // Move to the next 30-minute interval
                $startTime = $interval_end;
            }
        }

        // Sort the split intervals based on the keys ( time )
        ksort( $split_intervals );

        // Process the sorted intervals and output combined data
        foreach ( $split_intervals as $interval => $interval_data ) {
            // Process $interval_data for each 30-minute interval
            // Combine and output the data within the interval

            list($start_time, $end_time) = explode('-', $interval); // Extracting the start time
                    
            foreach ( $interval_data as $row ) {
                ...
                // split the time based on service time
                ...
                if ( isset( $_GET[ 'päivämäärä' ] ) && !empty( $_GET[ 'päivämäärä' ] ) ) {
                    ...
                    if ( $appointment_data ) {
                        foreach ( $appointment_data as $appointment ) {
                            if ( date( 'Y-m-d '.$startTime, strtotime( $_GET[ 'päivämäärä' ] ) ) == date( 'Y-m-d H:i', strtotime( $appointment->bookingStart ) + 2 * 60 * 60 ) ) {
                                $break_loop = true;
                            }
                        }
                    }
                    ...
                    if ( $appointment_data ) {
                        foreach ( $appointment_data as $appointment ) {
                            if ( date( 'Y-m-d '.$startTime ) == date( 'Y-m-d H:i', strtotime( $appointment->bookingStart ) + 2 * 60 * 60 ) ) {
                                $break_loop = true;
                            }
                        }
                    }
